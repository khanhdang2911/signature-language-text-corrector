...
const getHisoryById = async (req, res) => {
  try {
...
const createTextVoice = async (req, res) => {
  try {
    const data = req.body;
    if (!data.text_voice || !data.userId) {
      return res.status(500).json({
        success: false,
        message: "Missing required parameters!",
      });
    ...
    await connection.query(
      "insert into histories(text_voice, date_insert, userId) values (?,?,?)",
      [data.text_voice, formatDate, data.userId]
    );
    return res.status(200).json({
      success: true,
...
const updateTextVoice = async (req, res) => {
  try {
    ...
    await connection.query(
      "update histories set text_voice=?, date_insert=? where id=? ",
      [data.text_voice, formatDate, data.id]
    );
    return res.status(200).json({
      success: true,
